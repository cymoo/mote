...
func (h *TagHandler) GetTags(ctx context.Context) ([]models.TagWithPostCount, error) {
	tags, err := h.tagService.GetAllWithPostCount(ctx)
	if err != nil {
...
func (h *TagHandler) RenameTag(ctx context.Context, payload m.JSON[models.RenameTagRequest]) (m.StatusCode, error) {
	...
	err := h.tagService.RenameOrMerge(ctx, oldName, newName)
	if err != nil {
...
func (h *TagHandler) DeleteTag(ctx context.Context, payload m.JSON[models.Name]) (m.StatusCode, error) {
	tagName := payload.Value.Name
	err := h.tagService.DeleteAssociatedPosts(ctx, tagName)
	if err != nil {
...
// It returns a 204 No Content status on success.
func (h *TagHandler) StickTag(ctx context.Context, payload m.JSON[models.StickyTagRequest]) (m.StatusCode, error) {
	tagName := payload.Value.Name
	err := h.tagService.InsertOrUpdate(ctx, tagName, payload.Value.Sticky)
	if err != nil {
