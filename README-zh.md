...
当前分支是 **`feat/api-py`**。后端构建与运行见 [api-py/README.md](./api-py/README.md)；[frontend/](./frontend) 前端为各版本共享。

## 部署

生产环境部署参见 [deploy/](./deploy)，提供 nginx + Docker Compose 方案，含 HTTPS 证书自动申请和数据备份。注意：部署脚本目前构建的是 Go 后端（`deploy/Dockerfile.api`、`deploy/compose.yml`），本版本需相应改为构建 `api-py`。
